...
import { GenerateSubtitleRequest, GenerateSummaryRequest } from '../models/request.ts'

interface SideMenuProps {
  renewTaskStatus: () => void
  onGenerateSubtitleClick: (request: GenerateSubtitleRequest) => void
  onGenerateSummaryClick: (request: GenerateSummaryRequest) => void
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit}>
        ...
        {selectedFile && (
          <Text maxW="100%" whiteSpace="normal" overflowWrap="break-word" wordBreak="break-word">
            {selectedFile.name}
          </Text>
        )}

        <Button type="button" colorScheme="green" onClick={handleUploadClick} mb={2}>
          Select file
        </Button>
      ...
      <Select value={llmProvider} onChange={(e) => setLlmProvider(e.target.value)} mb={2}>
        {supportLLMs.map((llm) => {
          return (
      ...
      <Text>LLM Provider</Text>
      <Select value={llmModel} onChange={(e) => setLlmModel(e.target.value)} mb={2}>
        {supportLLMs
          .filter((llm) => llm.provider === llmProvider)
          .flatMap((llm) =>
      ...
      <Select value={whisperModel} onChange={(e) => setWhisperModel(e.target.value)} mb={2}>
      ...
      <Select value={whisperLang} onChange={(e) => setWhisperLang(e.target.value)} mb={2}>
